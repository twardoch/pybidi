...
class FRIBIDI:
    "useful definitions adapted from fribidi/*.h. You will need to use the constants and “macro” functions, but apart from that, see the more Pythonic wrappers defined outside this class in preference to accessing low-level structures directly."
    ...
    BIDI_MAX_EXPLICIT_LEVEL = (
        61  # maximum embedding level value assigned by explicit marks
    )
    BIDI_MAX_RESOLVED_LEVELS = (
        63  # maximum *number* of different resolved embedding levels: 0-62
    )
    ...
    def LEVEL_TO_DIR(lev):
        ...
        return (FRIBIDI.TYPE_LTR, FRIBIDI.TYPE_RTL)[FRIBIDI.LEVEL_IS_RTL(lev)]

    # end LEVEL_TO_DIR

    def DIR_TO_LEVEL(dir):
        "returns the minimum level of the direction, 0 for FRIBIDI.TYPE_LTR and1 for FRIBIDI.TYPE_RTL and FRIBIDI.TYPE_AL."
        return (0, 1)[FRIBIDI.IS_RTL(dir)]

    # end DIR_TO_LEVEL

    def IS_RTL(p):
        "Is right to left: RTL, AL, RLE, RLO?"
        return p & FRIBIDI.MASK_RTL != 0

    # end IS_RTL

    def IS_ARABIC(p):
        "Is arabic: AL, AN?"
        return p & FRIBIDI.MASK_ARABIC != 0

    # end IS_ARABIC

    def IS_STRONG(p):
        "Is strong?"
        return p & FRIBIDI.MASK_STRONG != 0

    # end IS_STRONG

    def IS_WEAK(p):
        "Is weak?"
        return p & FRIBIDI.MASK_WEAK != 0

    # end IS_WEAK

    def IS_NEUTRAL(p):
        "Is neutral?"
        return p & FRIBIDI.MASK_NEUTRAL != 0

    # end IS_NEUTRAL

    def IS_SENTINEL(p):
        "Is sentinel?"
        return p & FRIBIDI.MASK_SENTINEL != 0

    # end IS_SENTINEL

    def IS_LETTER(p):
        "Is letter: L, R, AL?"
        return p & FRIBIDI.MASK_LETTER != 0

    # end IS_LETTER

    def IS_NUMBER(p):
        "Is number: EN, AN?"
        return p & FRIBIDI.MASK_NUMBER != 0

    # end IS_NUMBER

    def IS_NUMBER_SEPARATOR_OR_TERMINATOR(p):
        "Is number separator or terminator: ES, ET, CS?"
        return p & FRIBIDI.MASK_NUMSEPTER != 0

    # end IS_NUMBER_SEPARATOR_OR_TERMINATOR

    def IS_SPACE(p):
        "Is space: BN, BS, SS, WS?"
        return p & FRIBIDI.MASK_SPACE != 0

    # end IS_SPACE

    def IS_EXPLICIT(p):
        "Is explicit mark: LRE, RLE, LRO, RLO, PDF?"
        return p & FRIBIDI.MASK_EXPLICIT != 0

    # end IS_EXPLICIT

    def IS_SEPARATOR(p):
        "Is text separator: BS, SS?"
        return p & FRIBIDI.MASK_SEPARATOR != 0

    # end IS_SEPARATOR

    def IS_OVERRIDE(p):
        "Is explicit override: LRO, RLO?"
        return p & FRIBIDI.MASK_OVERRIDE != 0
    ...
    def IS_LTR_LETTER(p):
        ...
        return p & (FRIBIDI.MASK_LETTER | FRIBIDI.MASK_RTL) == FRIBIDI.MASK_LETTER
    ...
    def IS_RTL_LETTER(p):
        ...
        return (
            p & (FRIBIDI.MASK_LETTER | FRIBIDI.MASK_RTL)
            == FRIBIDI.MASK_LETTER | FRIBIDI.MASK_RTL
        )
    ...
    def IS_PRIVATE(p):
        ...
        return p & FRIBIDI.MASK_PRIVATE != 0
    ...
    def CHANGE_NUMBER_TO_RTL(p):
        ...
        return (p, FRIBIDI.TYPE_RTL)[FRIBIDI.IS_NUMBER(p)]
    ...
    def WEAK_PARAGRAPH(p):
        ...
        return FRIBIDI.PAR_WLTR | p & FRIBIDI.MASK_RTL
    ...
    def JOINS_RIGHT(p):
        ...
        return p & FRIBIDI.MASK_JOINS_RIGHT != 0

    # end JOINS_RIGHT

    def JOINS_LEFT(p):
        "Joins to left: L, D, C?"
        return p & FRIBIDI.MASK_JOINS_LEFT != 0

    # end JOINS_LEFT

    def ARAB_SHAPES(p):
        "May shape: R, D, L, T?"
        return p & FRIBIDI.MASK_ARAB_SHAPES != 0

    # end ARAB_SHAPES

    def IS_JOIN_SKIPPED(p):
        "Is skipped in joining: T, G?"
        return p & (FRIBIDI.MASK_TRANSPARENT | FRIBIDI.MASK_IGNORED) != 0
    ...
    def JOINS_PRECEDING_MASK(level):
        return (FRIBIDI.MASK_JOINS_LEFT, FRIBIDI.MASK_JOINS_RIGHT)[
            FRIBIDI.LEVEL_IS_RTL(level)
        ]

    # end JOINS_PRECEDING_MASK

    def JOINS_FOLLOWING_MASK(level):
        return (FRIBIDI.MASK_JOINS_RIGHT, FRIBIDI.MASK_JOINS_LEFT)[
            FRIBIDI.LEVEL_IS_RTL(level)
        ]
    ...
    def JOIN_SHAPE(p):
        return p & (FRIBIDI.MASK_JOINS_RIGHT | FRIBIDI.MASK_JOINS_LEFT) != 0
...
fribidi.fribidi_get_par_direction.argtypes = (
    ct.POINTER(FRIBIDI.CharType),
    FRIBIDI.StrIndex,
)
...
fribidi.fribidi_get_joining_types.argtypes = (
    ct.POINTER(FRIBIDI.Char),
    FRIBIDI.StrIndex,
    ct.POINTER(FRIBIDI.JoiningType),
)
...
def str_to_chars(s):
    ...
    return seq_to_ct(s, FRIBIDI.Char, ord)
...
def get_unicode_version():
    ...
    return ct.c_char_p.in_dll(fribidi, "fribidi_unicode_version").value.decode()
...
def get_bidi_type(ch):
    ...
    return fribidi.fribidi_get_bidi_type(ch)
...
def get_bidi_type_name(t):
    ...
    return fribidi.fribidi_get_bidi_type_name(t).decode()
...
def get_par_direction(bidi_types):
    ...
    return fribidi.fribidi_get_par_direction(c_bidi_types, len(bidi_types))
...
class Reordering:
    ...
    def __init__(self, length, elt_func):
        "length is the number of elements, and elt_func is a function which, given each input index, returns the appropriate remapped index."
        self.indexes = tuple(elt_func(i) for i in range(length))
        assert set(self.indexes) == set(range(length)), (
            "reordering omits or duplicates items"
        )
    ...
    def identity(celf, length):
        "constructs an identity Reordering of the specified length. Instead of an integer, you can pass a sequence (bytes, list, str, tuple), and the length will be taken from that of the sequence."
        if isinstance(length, (bytes, list, str, tuple)):
            length = len(length)
        # end if
        return celf(length=length, elt_func=lambda i: i)

    # end identity

    @classmethod
    def from_seq(celf, seq):
        "constructs a Reordering taking its indexes from the specified sequence. Besides a list or tuple, you can also pass a ctypes array, for example of FRIBIDI.StrIndex elements."
        return celf(length=len(seq), elt_func=lambda i: seq[i])
    ...
    def inverse(self):
        ...
        # end for
        return type(self).from_seq(indexes)
    ...
    def apply(self, seq, offset=0):
        "applies the Reordering to the specified sequence (bytes, list, string, tuple) beginning at the specified offset. Any part of the sequence prior to the offset, and beyond the length of the Reordering, remains unchanged."
        if not isinstance(seq, (bytes, list, str, tuple)):
            raise TypeError(
                f"invalid type “{type(seq).__name__}” for applying Reordering"
            )
        # end if
        if len(seq) < offset + len(self):
            raise IndexError(
                "sequence length {} is too short for my Reordering length {} at offset {}".format(
                    len(seq), len(self), offset
                )
            )
        # end if
    ...
    def to_ct(self):
        ...
        return seq_to_ct(self.indexes, FRIBIDI.StrIndex)

    # end to_ct

    def __repr__(self):
        return f"<{type(self).__name__}>{repr(self.indexes)}"
...
def reorder_line(
    ...
    for seq, seq_name in (
        ...
        assert para_len == len(seq), f"lengths of bidi_types and {seq_name} disagree"
    # end for
    c_bidi_types = seq_to_ct(bidi_types, FRIBIDI.CharType)
    ...
    if map is not None:
        if isinstance(map, Reordering):
            assert len(map) == para_len, (
                "length of Reordering map disagrees with para len"
            )
            c_map = map.to_ct()
        elif isinstance(map, (list, tuple)):
            assert len(map) == para_len, (
                "length of map list/tuple disagrees with para len"
            )
...
def get_joining_type(ch):
    ...
    return fribidi.fribidi_get_joining_type(ch)
...
def get_joining_type_name(j):
    ...
    return fribidi.fribidi_get_joining_type_name(j).decode()
...
def join_arabic(bidi_types, embedding_levels, ar_props):
    ...
    return type(ar_props)(c_ar_props)
...
def shape_arabic(flags, embedding_levels, ar_props, string):
    ...
    c_ar_props = seq_to_ct(ar_props, FRIBIDI.ArabicProp)
    c_str = str_to_chars(string)
    fribidi.fribidi_shape_arabic(
        flags, c_embedding_levels, len(string), c_ar_props, c_str
    )
    return type(ar_props)(c_ar_props), chars_to_str(c_str)
...
def unicode_to_charset(char_set, us):
    ...
    if s_len >= len(s_bytes):
        raise IndexError("PANIC! buffer overflow!")  # if I haven’t crashed first...
    # end if
    return s_bytes.value  # automatically stops at NUL
...
def parse_charset(s):
    ...
    c_s = s.encode()
    return fribidi.fribidi_parse_charset(c_s)
...
def char_set_name(char_set):
    return fribidi.fribidi_char_set_name(char_set).decode()
...
def char_set_title(char_set):
    return fribidi.fribidi_char_set_title(char_set).decode()
...
def log2vis(
    ...
    # end if
    max_level = fribidi.fribidi_log2vis(
        c_string,
        strlen,
        ct.byref(c_pbase_dir),
        c_vis_string,
        c_positions_L_to_V,
        c_positions_V_to_L,
        c_embedding_levels,
...
def each_embedding_run(vis_line, embedding_levels, vis_order):
    "Generator function which yields in turn each contiguous run of the string vis_line (previously reordered by FriBidi) which has the same embedding level. Each result is a 4-tuple:\n\n     (substr, startindex, endindex, embedding_level)\n\nwhere substr is vis_line[startindex:endindex] and embedding_level is the corresponding embedding level for the entire substring. The segments are always returned in visual order (for rendering in turn left-to-right), and internally in visual order if vis_order, else they are internally rearranged to logical order (which is what HarfBuzz wants for shaping, for example)."
...
class ReorderLine:
    "convenience wrapper for a common use case: reorder a line of text and extract the embedding runs. Instantiate with the text line to be processed and the base direction (FRIBIDI.PAR_LTR or FRIBIDI.PAR_RTL). Then call the each_embedding_run() method to iterate over the embedding runs."

    def __init__(self, text_line, base_dir, flags=FRIBIDI.FLAGS_DEFAULT):
        self.text_line = text_line
        self.bidi_types = get_bidi_types(text_line)
        self.base_dir, self.embedding_levels = get_par_embedding_levels(
            self.bidi_types, base_dir
        )[1:]
    ...
    def each_embedding_run(self, vis_order):
        "generator function which yields in turn each contiguous run of the string line which has the same embedding level. Each result is a 4-tuple:\n\n     (substr, startindex, endindex, embedding_level)\n\nwhere substr is line[startindex:endindex] and embedding_level is the corresponding embedding level for the entire substring."
        return each_embedding_run(self.vis_line, self.vis_embedding_levels, vis_order)
